...
import { quoteFields } from '../../features/quote/data/fields';
...
export default function QuoteDetails() {
  ...
  const handleInputChange = (e: { target: { name: any; value: any } }) => {
    ...
    if (value !== initialData[name]) {
      ...
      if (
        (value && quoteFields[name].required) ||
        !quoteFields[name].required
      ) {
        setUpdatedFields({
          ...updatedFields,
          [name]: value,
  ...
  const handleUpdateProducts = async (items: any[]) => {
    try {
      ...
      // Check for added or removed quote items
      ...
      // Create an array of quote item objects to add to the updated quote
      const addedQuoteItems = addedItems.map((item) => ({
        product: { connect: { id: item.product.id } },
        quantity: parseInt(item.quantity, 10),
        subTotal: item.subTotal,
      }));

      // Create an array of quote item IDs to remove from the updated quote
      const removedQuoteItemIds = removedItems.map((item: any) => item.id);
      updatedFieldsCopy = {
        ...updatedFieldsCopy,
        totalAmount,
        items: {
          create: addedQuoteItems, // Add newly added items
          deleteMany: { id: { in: removedQuoteItemIds } }, // Remove removed items
        },
      };
      console.log('addedQuoteItems');
      console.log(addedQuoteItems);
      console.log('removedQuoteItemIds');
      console.log(removedQuoteItemIds);
      ...
      getAndUpdateDashboardItems('quote');
    } catch (error) {
  ...
  const handleUpdate = async () => {
    ...
    const requiredFields = Object.keys(quoteFields).filter(
      (fieldName) => quoteFields[fieldName].required
    );
  ...
  const handleOrderGeneration = async () => {
    if (
      currentSubStage?.actionTypeId === 2 ||
      currentStage?.actionTypeId === 2
    ) {
      // Generate new sale order
      console.log('Generate new sale order');
      try {
        retrieveLastOrder().then(async (orders) => {
          const orderId = orders?.records[0]?.id || 1; // Use optional chaining to safely access the ID
          const orderName = `S${orderId.toString().padStart(5, '0')}`;
          ...
          let newRecordData: any = {
            ...
            subStage: {
              connect: {
                id: orderStages[0].children.length
                  ? orderStages[0].children.sort(
          ...
          // remove the id key before saving
          ...
          await createRecord({
            ...
            createRecord({
              modelName: 'Log',
              data: {
                content: "Order was created successfully",
                userId: parseInt(localStorage.getItem('loggedInUserId') || '', 10),
                modelName: "Order",
  ...
  const handleUpdateStage = async (offset: number) => {
    try {
      // verify if the action is Sale Order Generation then generate new sale order based on quote data
      ...
      if (newSubStage) {
        ...
        setToastData({
          title: `Quote is now at ${newSubStage?.name} stage`,
          type: 'info',
        });
      } else {
        updates = {
        ...
        setToastData({
          title: `Quote is now at ${newStage?.name} stage`,
          type: 'info',
        });
      }
      await updateRecord.mutateAsync({
                ...
                <Card>
                  ...
                  >
                    <TabPanel activeIndex={activeIndex} index={0}>
                      <NotePanel modelName="Quote" recordId={data.id} />
                    </TabPanel>
                  ...
                  <TabPanel activeIndex={activeIndex} index={3}>
                    <LogPanel modelName="Quote" recordId={data.id} />
                  </TabPanel>
                  </Tab>
                </Card>
              </div>
