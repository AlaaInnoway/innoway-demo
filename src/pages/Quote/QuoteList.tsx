...
export default function QuoteList() {
  ...
  const handleDelete = async (id: number) => {
    const record = {
      modelName: 'quote', // Replace with the desired model name
      id,
    };
    ...
    try {
      ...
      setToastData({
        title: 'Quote have been deleted successfully',
        type: 'success',
      });
  ...
  const quoteActionMenus = [
    {
      id: 1,
      name: 'View',
      icon: <EyeIcon width={16} height={16} />,
      handleClick: (id: number) => navigate(`/sales/quote/${id}`),
    },
    {
      id: 2,
      name: 'Edit',
      icon: <PencilSquareIcon width={16} height={16} />,
      handleClick: (id: number) => navigate(`/sales/quote/${id}?edit=true`),
    },
    {
      id: 3,
      name: 'Remove',
  ...
  return (
    <Container>
          ...
          {openDialog && (
            <Dialog
              title="Confirmation"
              message="Are you really want to delete this quote?"
              open={openDialog}
              discardButton="No"
              confirmButton="Yes"
          ...
          <Card>
            <div className="space-y-2">
              ...
              >
                {data?.records?.map((quote: any, index: number) => (
                  <tr
                  ...
                  >
                    ...
                    {quote.organization && (
                      <td className="flex items-center py-4 px-6">
                        <Avatar
                          ...
                          src={
                            quote.organization.img &&
                            API_BASE_URL + quote.organization.img
                          }
                        />
                        <div className="pl-3">{quote.organization?.name}</div>
                      </td>
                    )}
                    {quote.individual && (
                      <td className="flex items-center py-4 px-6">
                        <Avatar
                          icon={<div>{quote.individual?.name[0]}</div>}
                          src={
                            quote.individual.img &&
                            API_BASE_URL + quote.individual.img
                          }
                        />
                        <div className="pl-3">{quote.individual?.name}</div>
                      </td>
                    )}
                    <td className="py-4 px-6 whitespace-nowrap">
                      <div>{quote.opportunity?.name}</div>
                    </td>
                    <td className="py-4 px-6 whitespace-nowrap">
                    ...
                    {visibleColumns.includes('expiredAt') && (
                      <td className="py-4 px-6 whitespace-nowrap">
                        <div>
                          {quote.expiredAt &&
                            new Date(quote.expiredAt).toLocaleDateString()}
                        </div>
                      </td>
                    )}
                    <td className="py-4 px-6">
                      <Avatar
                        icon={<div>{quote.user?.name[0]}</div>}
                        src={quote.user.img && API_BASE_URL + quote.user.img}
