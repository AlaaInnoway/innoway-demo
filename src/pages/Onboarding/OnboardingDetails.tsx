...
export default function OnboardingDetails() {
  ...
  const handleInputChange = (e: { target: { name: any; value: any } }) => {
    ...
    if (value !== initialData[name]) {
      ...
      if (
        (value && onboardingFields[name].required) ||
        !onboardingFields[name].required
      ) {
        setUpdatedFields({
          ...updatedFields,
          [name]: value,
  ...
  const handleSelectChange = (item: any, name: string) => {
    ...
    if (item.id !== initialData[name]) {
      // check if the input is filled and required or the field is not required, in this case update changes
      if (
        (item.id && onboardingFields[name].required) ||
        !onboardingFields[name].required
      ) {
        setUpdatedFields({
          ...updatedFields,
    ...
    if (Object.keys(updatedFields).length !== 0) {
      try {
        ...
        await updateRecord.mutateAsync({
          modelName: 'Onboarding', // Model name
  ...
  const handleUpdateStage = async (offset: number) => {
    try {
      ...
      if (newSubStage) {
        ...
        setToastData({
          title: `Onboarding is now at ${newSubStage?.name} stage`,
          type: 'info',
        });
      } else {
        updates = {
        ...
        setToastData({
          title: `Onboarding is now at ${newStage?.name} stage`,
          type: 'info',
        });
      }
      await updateRecord.mutateAsync({
              ...
              <Card>
                ...
                >
                  <TabPanel activeIndex={activeIndex} index={0}>
                    <NotePanel modelName="Onboarding" recordId={data.id} />
                  </TabPanel>
                  ...
                  <TabPanel activeIndex={activeIndex} index={3}>
                    <LogPanel modelName="Onboarding" recordId={data.id} />
                  </TabPanel>
                </Tab>
              </Card>
            </div>
                  ...
                  <div className="grid grid-cols-3 gap-4">
                    ...
                    <div className="col-span-3">
                      <TextArea
                        ...
                        handleChange={handleInputChange}
                        required={onboardingFields.description.required}
                      />
                    </div>
                  </div>
                </div>
