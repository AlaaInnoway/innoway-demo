...
export default function RemoteDetails() {
  ...
  const handleInputChange = (e: { target: { name: any; value: any } }) => {
    ...
    if (value !== initialData[name]) {
      ...
      if (
        (value && remoteFields[name].required) ||
        !remoteFields[name].required
      ) {
        setUpdatedFields({
          ...updatedFields,
          [name]: value,
  ...
  const handleSelectChange = (item: any, name: string) => {
    ...
    if (item.id !== initialData[name]) {
      // check if the input is filled and required or the field is not required, in this case update changes
      if (
        (item.id && remoteFields[name].required) ||
        !remoteFields[name].required
      ) {
        setUpdatedFields({
          ...updatedFields,
  ...
  const handleDataUpdate = () => {
    ...
    if (data && id) {
      setInitialData(data);
      setRecordData(data);
      setRemoteBalance(data.employee?.remoteBalance);
    }
  };
  ...
  const handleUpdateStage = async (offset: number) => {
    try {
      ...
      if (newSubStage) {
        ...
        setToastData({
          title: `Remote Work request is now at ${newSubStage?.name} stage`,
          type: 'info',
        });
      } else {
        updates = {
        ...
        setToastData({
          title: `Remote Work request is now at ${newStage?.name} stage`,
          type: 'info',
        });
      }
      await updateRecord.mutateAsync({
              ...
              <Card>
                ...
                >
                  <TabPanel activeIndex={activeIndex} index={0}>
                    <NotePanel modelName="Remote" recordId={data.id} />
                  </TabPanel>
                  ...
                  <TabPanel activeIndex={activeIndex} index={3}>
                    <LogPanel modelName="Remote" recordId={data.id} />
                  </TabPanel>
                </Tab>
              </Card>
            </div>
