...
import TermList from '../../features/employee/components/TermList';
import BenefitList from '../../features/employee/components/BenefitList';
...
export default function ContractDetails() {
  ...
  const handleInputChange = (e: { target: { name: any; value: any } }) => {
    ...
    if (value !== initialData[name]) {
      ...
      if (
        (value && contractFields[name].required) ||
        !contractFields[name].required
      ) {
        setUpdatedFields({
          ...updatedFields,
          [name]: value,
  ...
  const handleSelectChange = (item: any, name: string) => {
    ...
    if (item.id !== initialData[name]) {
      // check if the input is filled and required or the field is not required, in this case update changes
      if (
        (item.id && contractFields[name].required) ||
        !contractFields[name].required
      ) {
        setUpdatedFields({
          ...updatedFields,
  ...
  const handleUpdateStage = async (offset: number) => {
    try {
      ...
      if (newSubStage) {
        ...
        setToastData({
          title: `Contract is now at ${newSubStage?.name} stage`,
          type: 'info',
        });
      } else {
        updates = {
        ...
        setToastData({
          title: `Contract is now at ${newStage?.name} stage`,
          type: 'info',
        });
      }
      await updateRecord.mutateAsync({
          ...
          <div className="grid grid-cols-3 gap-4">
              ...
              <Card>
                ...
                >
                  <TabPanel activeIndex={activeIndex} index={0}>
                    <NotePanel modelName="Contract" recordId={data.id} />
                  </TabPanel>
                  ...
                  <TabPanel activeIndex={activeIndex} index={3}>
                    <LogPanel modelName="Contract" recordId={data.id} />
                  </TabPanel>
                </Tab>
              </Card>
            </div>
            <div className="col-span-2 space-y-4">
              ...
              {recordData && (
                <Card>
                  <Tab
                    values={['Terms and Conditions', 'Benefits']}
