...
export default function InvoiceList() {
  ...
  const handleDelete = async (id: number) => {
    const record = {
      modelName: 'invoice', // Replace with the desired model name
      id,
    };
    ...
    try {
      ...
      setToastData({
        title: 'Invoice have been deleted successfully',
        type: 'success',
      });
  ...
  const invoiceActionMenus = [
    {
      id: 1,
      name: 'View',
      icon: <EyeIcon width={16} height={16} />,
      handleClick: (id: number) => navigate(`/sales/invoice/${id}`),
    },
    {
      id: 2,
      name: 'Edit',
      icon: <PencilSquareIcon width={16} height={16} />,
      handleClick: (id: number) => navigate(`/sales/invoice/${id}?edit=true`),
    },
    {
      id: 3,
      name: 'Remove',
  ...
  return (
    <Container>
      ...
      {openDialog && (
        <Dialog
          title="Confirmation"
          message="Are you really want to delete this invoice?"
          open={openDialog}
          discardButton="No"
          confirmButton="Yes"
      ...
      <Card>
        <div className="space-y-2">
          ...
          >
            {data?.records?.map((invoice: any, index: number) => (
              <tr
              ...
              >
                ...
                {invoice.organization && (
                  <td className="flex items-center py-4 px-6">
                    <Avatar
                      ...
                      src={
                        invoice.organization.img &&
                        API_BASE_URL + invoice.organization.img
                      }
                    />
                    <div className="pl-3">{invoice.organization?.name}</div>
                  </td>
                )}
                {invoice.individual && (
                  <td className="flex items-center py-4 px-6">
                    <Avatar
                      icon={<div>{invoice.individual?.name[0]}</div>}
                      src={
                        invoice.individual.img &&
                        API_BASE_URL + invoice.individual.img
                      }
                    />
                    <div className="pl-3">{invoice.individual?.name}</div>
                  </td>
                )}
                <td className="py-4 px-6 whitespace-nowrap">
                  <div>{invoice.opportunity?.name}</div>
                </td>
                <td className="py-4 px-6 whitespace-nowrap">
                ...
                {visibleColumns.includes('validityPeriod') && (
                  <td className="py-4 px-6">
                    <div>{invoice.validityPeriod} days</div>
                  </td>
                )}
                {visibleColumns.includes('expiredAt') && (
                  <td className="py-4 px-6 whitespace-nowrap">
                    <div>
                      {invoice.expiredAt &&
                        new Date(invoice.expiredAt).toLocaleDateString()}
                    </div>
                  </td>
                )}
                <td className="py-4 px-6">
                  <Avatar
                    icon={<div>{invoice.user?.name[0]}</div>}
                    src={
                      invoice.user.img && API_BASE_URL + invoice.user.img
                    }
                ...
                <td className="py-4 px-6 whitespace-nowrap">
                  <Badge
                    value={invoice.stage?.name}
                    color={invoice.stage?.color}
                  />
                </td>
                <td>
                  <DropdownAction
                    actionMenu={invoiceActionMenus}
                    recordId={invoice.id}
                  />
                </td>
              </tr>
